...
- **Exclusion lists** stop collection at the earliest gate (Doc 05 §4): match by process / window-class / title-regex / **`url_pattern`** (ADR-040); excluded contexts yield metadata-only events flagged `EXCLUDED` and can never appear in any payload. **Curated defaults are seeded once** [AMENDED 2026-08-16, decision #20 — supersedes ADR-029/Q15's empty-by-default]: password managers and banking patterns (`SHIPPED_DEFAULT_RULES`) are inserted into the durable `exclusion_list` at first launch, fully user-disableable/deletable, and never resurrect once deleted. **Detect-and-suggest** onboarding (§8) still runs on top for apps the curated list misses, and the one-click **"exclude this domain"** action on any browser bubble remains.
...
## Implementation status (2026-08-22) — SC5 is real; image redaction; the agent's egress is gated the same way

- **§2 two-emitter rule, proven at runtime.** `gates/tests/sc5_network_monitor.rs` is no longer a `todo!()` stub and no longer `#[ignore]`d: a loopback origin stands in for the API and counts bytes; `netstat` proves zero non-loopback TCP connections from the test process and a CIM query proves zero child spawns through the real proactive chain (pattern engine over a scripted stream → real redactor + payload builder over the golden fixture → staged preview); the approved Send's body SHA-256 == the preview hash == the `cloud_send` audit row; an unapproved payload never opens the socket. Loopback is whitelisted per ADR-028. **Note:** the lint sanctions *two* crates (`reasoning-gateway` for egress, `orchestration` for sidecar spawn + sanctioned model ingress); §2's "exactly one crate" wording predates that and should be read with doc 12 §3.
- **§3 the push path is transport-bound now** (2026-08-19 finding 1): a Send cannot leave over a transport other than the one the preview named — see doc 09.
- **§5 redaction covers images (owner decision #3):** `privacy::image_redaction::redact_bgra` + `Redactor::find_spans` (byte spans with the same gates as `redact_text`); solid block, not blur (blur is partially invertible). The only producer of a payload screenshot (`screen-serializer::observe_frame`) runs it before the JPEG exists.
- **§3 agent mode (Doc 22 §8):** every `aperture_agent_step` release writes a `cloud_send` row **before** releasing, fail-closed like `get_context`; the approval is once per task (decision #48) and stated on screen with what leaves per step; exclusions gate both observation (`observe_now`) and action (`ExclusionProbe`). Raw frames are returned to the serializer and dropped there — never persisted.
