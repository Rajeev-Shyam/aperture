...
| | |
|---|---|
| **Inputs** | OS frames (WGC), WinEvent/UIA notifications, foreground-window queries, toggle state (from Doc 12), exclusion list (Doc 13) |
...
| Browser URL | UIA read of the address-bar Edit element ("Address and search bar" in en-US Chrome — **language/version dependent, RK4**) on focus/title-change of a browser process | `navigation{url}` |
...
- **Heartbeat sample** every **10 s** [ASSUMPTION] while the user is active (input within 60 s); suspended when idle.
- Per-sample work: capture one frame of the foreground monitor → crop to foreground window rect when cheap → hand to OCR.
- **Exclusion enforcement happens here:** if the foreground process/window matches `exclusion_list`, no frame is captured, the event is recorded as metadata-only with `redaction_flags|=EXCLUDED`, and no OCR/connector capture runs. This is the earliest possible gate (Doc 13).
...
```
   ...
   └── 3. UnhookWinEvent / remove UIA handlers   STARTING: re-acquire WGC item/pool,
       4. signal Doc 12 → kill vlm-host/stt-host  re-register hooks, resume sampler
       5. flip tray + overlay indicator to ⏸      indicator ▶, emit capture_toggle(on)
       6. emit capture_toggle(off) audit event
```
- OFF guarantees: **no events written, no frames taken, sidecars dead, VRAM released** (verified at the M1 gate with `nvidia-smi`).
...
| UIA address-bar read fails (localization, browser update) | Fall back to last-known URL or skip `navigation`; connector marks state stale (RK4) |
