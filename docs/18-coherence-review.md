# Doc 18 — Coherence & Connection Review
...
- **FIX 2 (new in the split):** the L1 loadout means VLM+Whisper may be **co-resident** while the mutex still serializes **execution**; Docs 04 §4, 07 §3, 12 §3 now state the same residency-vs-execution distinction, and L2 is uniformly described as residency-exclusive with swap.
...
- **C4 (new, found in the split):** Doc 11 said "max 3 visible bubbles" while Doc 14 said "≤ 3 glass surfaces" — these are now explicitly the **same cap with two rationales** (UX + GPU), cross-referenced in both. Resolved.
