...
**Clarified (also locked):** (A) the proactive loop is **fully local**; Claude only on explicit user action, transport swappable across Claude Desktop MCP / Claude Code CLI / Messages API — **MCP is the primary** (ADR-025), all transport specifics [VERIFY]. (B) Voice = telemetry **and** query channel; answers render as **text** in a bubble (no TTS). (C) Stack chosen freely for the hardware budget → **Tauri shell, Rust core, llama.cpp/whisper-family sidecars, SQLite + sqlite-vec**. (D) **[R2] The browser extension is a committed v1 component** (ADR-027): a Manifest V3 extension (Chrome + Opera GX) is the *primary* source for browser URL (tabs API) and YouTube position (`video.currentTime`); UIA reading is the no-extension fallback. (E) **[R2; amended 2026-08-16] Scoped "always-allow"** (ADR-026) is **v2-only — descoped from v1**: it was never built in v1, and its dead `scoped_allow_*` settings keys were removed in the 2026-08-15 review so settings only advertise built behavior. **v1 ships strict per-send approval** — every cloud egress is an explicit Send on the exact rendered payload. The ADR-026 design carries forward unchanged as v2's per-task approval (Doc 22 §4.1 / V2-M3): the exact payload is still displayed, a cancel window precedes egress, and the SHA-256 is still audit-logged.
...
2. **The transparency gate** — **raw user data leaves only via the Reasoning Gateway crate, only after the user approves the exact serialized payload** (an explicit Send *or*, in v2 only, a scoped allow with payload-display + cancel window + audit — see clarified decision E). The gateway crate is the **only** code that opens application network sockets — including the opt-in, off-by-default, **aggregate-only diagnostics** path. The Tauri app-updater is a **separate framework path carrying no user-derived data**, excluded from the SC5 *user-data*-egress test but visible to the network monitor (ADR-026, ADR-036; Docs 09, 13).
